...
import { scan, tap, map, publishReplay, refCount } from 'rxjs/operators';
...
export class BfACLService {
  ...
  constructor() {
    this.rules$ = this.updateSubject.pipe(
      scan((rules: ACLType, mutaion: Mutation) => ({ ...mutaion(rules) }), {} as ACLType),
      tap(value => this.rules = value),
      publishReplay(1),
      refCount()
    );

    this.setSubject.pipe(map<ACLType, Mutation>(value => () => value))
      .subscribe(this.updateSubject);

    this.cleanSubject.pipe(map<any, Mutation>(() => () => ({})))
      .subscribe(this.updateSubject);
